...
#PROGRAM DESCRIPTION:
#This program allows for the user to create, edit, save, and load questions and answers for a 'play game' mode they can try
#The 'play game' mode allows for 1 or 2 players. They are asked questions by the computer and they have to try answering them correctly.
#All data (questions,answers etc.) in the program is stored within lists which are stored within one large 2 dimensional list that the program uses to store and access data. 


#Test cases that were used to make sure no errors occur
# COMMON INPUT CASES:
# "abc", "1", "0"
#Variables tested with these cases: 'questionsCount', 'correctAns', 'retry', 'questionToEdit', 'usersAnswer', 'selectMode', 'continuation'
#                                   'questionsPreview'

# EXTREME INPUT CASES:
# "(blank)", " (space)", "-2", "ctrl"+"c"
#Variables tested with these cases: 'inputQuestion', 'questionsCount', 'correctAns', 'inputAnswer', 'questionToEdit', 'retry',
...
def edit_question():
    try:
        questionToEdit=int(input("Which question number do you want to change? (Enter referral index for that question)"))
        ...
        if confirmation==1:
            ...
            newData[5]=str(newData[5])
            for x in range(6):
                allLists[x][questionToEdit]=newData[x]
...
def save_game():
    try:
        ...
        savedFile=open("gameSaveFil.txt","w")
...
def load_game():
    try:
        savedFile=open("gameSaveFil.txt","r")
